...
public class ModeloVistaJuego implements IModeloVista {
    ...
    private boolean activarJugadaEspecial = false;
    ...
    @Override
    public double actualizarPuntaje(int puntos) {
        jugadorPrincipal.addPuntos(puntos);
        this.marcador = (int) jugadorPrincipal.getPuntaje();
        notificar();
        return jugadorPrincipal.getPuntaje();

    }

    @Override
    public void actualizarTarjetaJugadorecundario(int puntos, String jugada, int idJugador) {
        
            if (jugadoresSecundarios != null) {
            for (JugadorVista j : jugadoresSecundarios) {
                if (idJugador == j.getNumJugador()) {
                    j.addPuntos(puntos);
                        this.activarJugadaEspecial = true;
                        notificar();
                        this.activarJugadaEspecial = false;
                    }
                }
            }
        }
    ...
    @Override
    public boolean isActivarJugadaEspecial() {
        return activarJugadaEspecial;
    }
}
