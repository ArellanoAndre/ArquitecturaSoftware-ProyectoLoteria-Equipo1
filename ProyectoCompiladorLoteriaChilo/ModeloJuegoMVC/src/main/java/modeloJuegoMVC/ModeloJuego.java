...
public class ModeloJuego implements IModeloJuego {
    ...
    public void actualizarPuntuacion(int idJugador,int puntos, String jugada){
            
        System.out.println("[ModeloJuego] Jugada "+jugada+" -> Jugador: " + idJugador + ", puntos: " + puntos);

        // 1. CASO JUGADOR PRINCIPAL (TÚ)
        if (jugadorPrincipal.getNumJugador() == idJugador) {
            System.out.println("   -> Es el jugador PRINCIPAL. Actualizando...");
            jugadorPrincipal.addPuntos(puntos);
            controlVistaJuego.actualizarPuntos(puntos);
        } // 2. CASO JUGADORES SECUNDARIOS (RIVALES)
        else {
            System.out.println("   -> Es un jugador SECUNDARIO. Buscando en lista de tamaño: " + (jugadoresSecundarios != null ? jugadoresSecundarios.size() : "null"));
            ...
            if (jugadoresSecundarios != null) {
                for (IJugador rival : jugadoresSecundarios) {
                    ...
                    if (rival.getNumJugador() == idJugador) {
                        System.out.println("   -> Rival ENCONTRADO (ID " + rival.getNumJugador() + "). Marcando casilla...");

                        // A. Actualizamos la entidad
                        rival.addPuntos(puntos);
                        controlVistaJuego.actualizarVistaJugadas(puntos, jugada, idJugador);
                        //controlVistaJuego.actualizarTarjetaJugadorecundario(int puntos, String jugada);
                        encontrado = true;
                        break; 
                    }
                }
            ...
            if (!encontrado) {
                System.err.println("   -> [ALERTA] No se encontró ningún jugador secundario con ID: " + idJugador);
            }
        }
    }
